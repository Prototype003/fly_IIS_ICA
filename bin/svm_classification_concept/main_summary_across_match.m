...
%% Load accuracies (for getting big-phi accuracy)
...
for const_type_c = 1 : length(const_types)
    ...
    results_file = ['4ch_phi3Concept_unpart_svm_' class_type '.mat'];
...
o_accuracies = struct(); % accuracies for each individual concept

results_dir = 'results/';
for const_type_c = 1 : length(const_types)
    const_type = const_types{const_type_c};
    results_file = ['4ch_phi3Concept_matchOrder_' const_type '_svm_' class_type '.mat'];
    acc_tmp = load([results_dir results_file]);
    
    % Take the values at a specific cost
    cost_param = 1;
    o_accuracies.(const_type) = permute(mean(acc_tmp.cost_accuracies(:, :, :, cost_param), 2), [1 3 4 2]);
    
    % Take the maximum across costs
    o_accuracies.(const_type) = permute(mean(max(acc_tmp.cost_accuracies, [], 4), 2), [1 3 2]);
...
for const_type_c = 1 : length(const_types)
    ...
    % Take the values at a specific cost
    cost_param = 1;
    comp_accuracies.(const_type) = mean(acc_tmp.cost_accuracies(cost_param, :, :), 3);
...
% Full constellation results
comp_acc_all = squeeze(mean(comp_accuracies.(const_type), 3))';
