function [] = main_classify_concept_match(match_orders, constellation_type, class_type)
...
%
% Inputs:
%   match_orders = vector of concept orders to use for classification
%
% Outputs:
%
...
results_file = [num2str(nChannels) 'ch_phi3Concept_matchOrder_' constellation_type '_svm_' class_type '.mat'];

cost_powers = (-20:10:20);
costs = 2 .^ cost_powers;

% flies x networks x concepts+Phi x costs
cost_accuracies = zeros(size(big_mips, 2), size(big_mips, var_source_dim), length(match_orders), length(costs));
...
order_concepts = {(1:4), (5:10), (11:14), (15)}; % hardcoded for 4-channel constellation
...
save([results_location results_file], 'cost_accuracies', 'costs', 'cost_powers', 'nChannels', 'tau', 'match_orders');
