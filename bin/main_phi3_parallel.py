...
def parallel_fly(fly):
	global phi_threes, state_counter, state_phis, state_trajectories, tpms
			...
			for channel_set_counter in range(0, len(channel_sets)):
				...
				for bins_counter in range(0, len(nBins)):
					...
					for trial in range(0, fly_data.shape[2]):
						...
						# Calculate phi for each state which occurred (i.e. for states where state_counter > 0)
						state_phis[:, channel_set_counter, trial, fly, condition, tau_counter, bins_counter] = None # To keep track of states which never occurred
						phi_total = 0
						for state_index in range(0, len(state_counter)):
							if state_counter[state_index, channel_set_counter, trial, fly, condition, tau_counter, bins_counter] > 0:
								...
								# Compute phi
								state_phis[state_index, channel_set_counter, trial, fly, condition, tau_counter, bins_counter] = pyphi.compute.big_phi(subsystem)
								
								# Add phi to total, weighted by the number of times the state occurred
								phi_total += state_phis[state_index, channel_set_counter, trial, fly, condition, tau_counter, bins_counter] * state_counter[state_index, channel_set_counter, trial, fly, condition, tau_counter, bins_counter]
						phi_threes[channel_set_counter, trial, fly, condition, tau_counter, bins_counter] = phi_total / np.sum(state_counter)
...
for nChannels_counter in range(0, len(nChannels)):
	...
	# Storage of state counts and phis (states x channel sets x trials x flies x conditions x taus x nBins)
	...
	for fly in flies:
		fly_parallelised = multiprocessing.Process(target=parallel_fly, args=(fly,))
		fly_parallelised.start()
		fly_parallelised.join()
