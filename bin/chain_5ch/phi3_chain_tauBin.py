...
results_file_suffix = "_chainedPhi_f" + "{0:0>2}".format(fly) + "tauBin" + str(tau) + "t" + str(trial) 
...
# Load data (load the full, preprocessed data file)
data_file = data_file_prefix + ".mat"
loaded_data = load_mat(data_directory + data_file)
fly_data = loaded_data['fly_data']
print("Data loaded")

# Extract channels of interest for fly of interest
fly_data = fly_data[:, channel_set, trial-1, fly-1, :] # Inputs to the script are 1 indexed

# Resample (average LFPs into tau bins)
fly_data = tau_resample(fly_data[:, :, None, None, :], tau)

# Binarise using median split
fly_data, tmp, tmp  = binarise_trial_median(fly_data)

for condition in range(0, nConditions):
	
	# Build TPM
	tpm = build_tpm(fly_data[:, :, 0, 0, condition, None], tau, n_values)
	tpms[:, :, condition] = tpm
	print("TPM built")
	...
	for sample_counter in range(0, fly_data.shape[0]):
		sample = fly_data[sample_counter, :, 0, 0, condition]
