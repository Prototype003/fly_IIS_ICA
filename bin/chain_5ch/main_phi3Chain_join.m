...
source_directory = [bin_location 'results_phiChain_split/'];
...
output_directory = [bin_location 'results_phiChain/'];
if ~isdir(output_directory)
    mkdir(output_directory);
end
output_file = [source_prefix source_suffix];
...
for fly = 1 : length(flies)
    ...
    for trial = 1 : length(trials)
        file_id = ['_f' num2str(fly, '%02d') 'tau' num2str(tau) 't' num2str(trial)];
...
phi_joined = struct();
phi_joined.channel_sets = channel_sets;
