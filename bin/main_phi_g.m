...
%{
This script computes phi_g for a combinations of n channels, for some
set of taus.

Based off main_phi_star.m
...
results_filename = [data_file '_detrend' num2str(prep_detrend) '_zscore' num2str(prep_zscore) '_nChannels' num2str(nChannels(1)) 't' num2str(nChannels(end)) '_medianSplit' num2str(prep_medianSplit) '_phig' '_allPartitions']; %flies' num2str(flies(1)) 't' num2str(flies(end))];
...
for nChannels_counter = 1 : length(nChannels)
    ...
    phi_gs = zeros(size(channel_sets, 1), size(fly_data, 3), size(fly_data, 4), size(fly_data, 5), length(taus), length(nBins));
    phi_gs_normalised = zeros(size(channel_sets, 1), size(fly_data, 3), size(fly_data, 4), size(fly_data, 5), length(taus), length(nBins));
    ...
    partitions_phi_gs = zeros(Bell(channels)-1, size(channel_sets, 1), size(fly_data, 3), size(fly_data, 4), size(fly_data, 5), length(taus), length(nBins));
    partitions_phi_gs_normalised = zeros(Bell(channels)-1, size(channel_sets, 1), size(fly_data, 3), size(fly_data, 4), size(fly_data, 5), length(taus), length(nBins));
                    ...
                    for condition = 1 : size(fly_data, 5)
                        for trial = 1 : size(fly_data, 3)
                            ...
                            [phi_tmp,...
                                ...
                                partitions_phi_tmp,...
                                ] = phig_mip(cov_past_past, cov_present_past, cov_present_present, channel_sets(channel_set, :));
                            
                            phi_gs(channel_set, trial, fly, condition, tau_counter, nBins_counter) = phi_tmp(1);
                            phi_gs_normalised(channel_set, trial, fly, condition, tau_counter, nBins_counter) = phi_tmp(2);
                            
                            partitions_phi_gs(:, channel_set, trial, fly, condition, tau_counter, nBins_counter) = partitions_phi_tmp(:, 1);
                            partitions_phi_gs_normalised(:, channel_set, trial, fly, condition, tau_counter, nBins_counter) = partitions_phi_tmp(:, 2);
    ...
    phis{nChannels_counter}.phi_gs = phi_gs;
    phis{nChannels_counter}.phi_gs_normalised = phi_gs_normalised;
    phis{nChannels_counter}.mips = mips;
    
    phis{nChannels_counter}.partitions = partitions;
    phis{nChannels_counter}.partitions_phi_gs = partitions_phi_gs;
    phis{nChannels_counter}.partitions_phi_gs_normalised = partitions_phi_gs_normalised;
