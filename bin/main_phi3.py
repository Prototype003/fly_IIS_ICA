...
for nChannels_counter in range(0, len(nChannels)):
	...
	state_partitions = np.empty((n_states, partitions_n, len(channel_sets), fly_data.shape[3], fly_data.shape[4], len(taus)), dtype=tuple)
	state_partitions_phis = np.zeros((n_states, partitions_n, len(channel_sets), fly_data.shape[3], fly_data.shape[4], len(taus)))
