...
for nChannels_counter in range(0, len(nChannels)):
	...
	mips = np.empty((n_states, len(channel_sets), fly_data.shape[3], fly_data.shape[4], len(taus)), dtype=tuple)
	...
	state_partitions = np.empty((n_states, possible_partitions[nChannels_counter], len(channel_sets), fly_data.shape[3], fly_data.shape[4], len(taus)), dtype=tuple)
	state_partitions_phis = np.zeros((n_states, possible_partitions[nChannels_counter], len(channel_sets), fly_data.shape[3], fly_data.shape[4], len(taus)))
			...
			for tau_counter in range(0, len(taus)):
				...
				for channel_set_counter in range(0, len(channel_sets)):
					...
					for state_index in range(0, n_states):
						...
						state_phis[state_index, channel_set_counter, fly, condition, tau_counter] = big_mip[0].phi
						mips[state_index, channel_set_counter, fly, condition, tau_counter] = big_mip[0].cut
						
						# Store phis for all partition schemes
						for partition_counter in range(0, possible_partitions[nChannels_counter]):
							state_partitions_phis[state_index, partition_counter, channel_set_counter, fly, condition, tau_counter] = big_mip[1][partition_counter].phi
							state_partitions[state_index, partition_counter, channel_set_counter, fly, condition, tau_counter] = big_mip[1][partition_counter].cut
					...
					for bins_counter in range(0, len(nBins)):
						...
						for trial in range(0, fly_data.shape[2]):
							...
							for sample_counter in range(0, fly_data.shape[0]):
								...
								state_counter[state, channel_set_counter, trial, fly, condition, tau_counter, bins_counter] += 1
							...
							for state_index in range(0, len(state_counter)):
								if state_counter[state_index, channel_set_counter, trial, fly, condition, tau_counter, bins_counter] > 0:
									# Add phi to total, weighted by the number of times the state occurred
									phi_total += state_phis[state_index, channel_set_counter, fly, condition, tau_counter, bins_counter] * state_counter[state_index, channel_set_counter, trial, fly, condition, tau_counter, bins_counter]
							phi_threes[channel_set_counter, trial, fly, condition, tau_counter, bins_counter] = phi_total / np.sum(state_counter[:, channel_set_counter, trial, fly, condition, tau_counter, bins_counter])
