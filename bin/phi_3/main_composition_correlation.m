...
for partitioned = 1 : 2
    ...
    for condition = 1 : size(concept_phi.twoCh, 4)
        x_vals = squeeze(mean(mean(concept_phi.twoCh(:, :, fly, condition, tau, partitioned), 2), 3));
        y_vals = squeeze(mean(mean(concept_phi.fourCh(:, :, fly, condition, tau, partitioned), 2), 3));
...
for partitioned = 1 : 2
    subplot(1, size(concept_phi.twoCh, 4), partitioned);
    x_vals = squeeze(mean(mean(concept_phi.twoCh(:, :, fly, 1, tau, partitioned)-concept_phi.twoCh(:, :, fly, 2, tau, partitioned), 2), 3));
    y_vals = squeeze(mean(mean(concept_phi.fourCh(:, :, fly, 1, tau, partitioned)-concept_phi.fourCh(:, :, fly, 2, tau, partitioned), 2), 3));
