...
# Results file
results_file_suffix = "_nChannels" + str(nChannels) + "_globalTPM" + str(global_tpm) + "_f" + "{0:0>2}".format(fly) + "c" + str(condition) + "tauBin" + str(tau) + "s" + "{0:0>4}".format(set) + "t" + str(trial)
...
# Build TPM
tpm = build_tpm(fly_data[:, channel_set, None], 1, n_values)
print("TPM built")
...
for state_index in range(0, n_states):
	...
	big_mips[state_index] = big_mip.to_json()
