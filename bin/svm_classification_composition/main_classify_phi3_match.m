function [] = main_classify_phi3_match(nChannels)
...
networks = nchoosek((1:15), nChannels);
networks_match = nchoosek((1:15), nChannels_match);
...
%% Determine which channel subsets correspond to which channel sets a priori

% nchannels choose 2 (coherence is always pairwise)
network_sets = zeros(size(networks_match, 1), nchoosek(size(networks_match, 2), nChannels));

for network_counter = 1 : size(networks_match, 1)
    network = networks_match(network_counter, :);
    
    % Every possible pair for this set of channels
    matching_sets = nchoosek(network, nChannels);
    
    % Find indices of each pair
    for set_counter = 1 : size(matching_sets, 1)
        set_id = matching_sets(set_counter, :);
        [match, index] = ismember(set_id, networks, 'rows'); % there should only be one match
        network_sets(network_counter, set_counter) = index;
    end
