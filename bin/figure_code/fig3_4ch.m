...
nCh = (4) - 1; % specify which channels to plot for
...
for nChannels_counter = nCh
    nChannels = phis{nChannels_counter}.nChannels;
    channel_sets = double(phis{nChannels_counter}.channel_sets);
    
    % Mean across trials, repeated t-tests with correction
    values = permute(mean(phis{nChannels_counter}.phis(:, :, flies, :, tau), 2), [1 3 4 2]);
    ps = zeros(size(values, 1), 1);
    hs = zeros(size(ps));
    for ch_set = 1 : size(values, 1)
        [ps(ch_set), hs(ch_set)] = signrank(squeeze(values(ch_set, :, 1)' - values(ch_set, :, 2)'), 0, 'tail', 'right'); % test at each channel set
    end
    sig_thresh = FDR(ps, 0.05); sigs = ps < sig_thresh; % FDR correction
    ...
    xCounter = 1;
    for value_type = 1 : size(plot_values, 2)
        subplot(length(nCh), size(plot_values, 2), subplot_counter);
...
figure_name = 'fig3_sig'; % 'fig3' for phi-3; 'fig3Sgauss' for phi-star-gaussian
