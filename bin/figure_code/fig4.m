...
for nChannels = 1 : length(accuracies)
    ...
    measure_accuracies_w = [measure_accuracies_w; permute(mean(values, 1), [2 1])];
    measure_groups_w = [measure_groups_w; zeros(size(values, 2), 1) + group_counter];
...
% %% Data for boxplots (within)
% % Averages across flies instead of sets for each fly
% 
% % Power
% results_filename = 'split2250_bipolarRerefType1_lineNoiseRemoved_power_classification.mat';
% load([results_directory results_filename]);
% measure_accuracies_w = permute(mean(mean(accuracies(freq_range, :, :), 1), 3), [2 1 3]); % average across frequency range and channels
% measure_groups_w = zeros(size(measure_accuracies_w)) + 1;
% 
% % Coherence
% results_filename = 'split2250_bipolarRerefType1_lineNoiseRemoved_coherence_classification.mat';
% load([results_directory results_filename]);
% measure_accuracies_w = [measure_accuracies_w; permute(mean(mean(accuracies(freq_range, :, :), 1), 3), [2 1 3])]; % average across frequency range and sets
% measure_groups_w = [measure_groups_w; zeros(size(accuracies, 2), 1) + 2];
% 
% % Phi-three
% results_filename = 'split2250_bipolarRerefType1_lineNoiseRemoved_phithree_nonGlobal_classification.mat';
% load([results_directory results_filename]);
...
% for nChannels = 1 : length(accuracies)
%     values = accuracies{nChannels}.accuracies;
%     measure_accuracies_w = [measure_accuracies_w; permute(mean(values, 2), [1 2])];
%     measure_groups_w = [measure_groups_w; zeros(size(values, 1), 1) + group_counter];
%     group_counter = group_counter + 1;
% end
% 
% % Phi-star
...
% 
% % Use with
% %distributionPlot(measure_accuracies_w, 'groups', measure_groups_w, 'addspread', 1)
...
for nChannels = 1 : length(accuracies)
    values = accuracies{nChannels}.accuracies;
    measure_accuracies_a = [measure_accuracies_a; values];
    measure_groups_a = [measure_groups_a; zeros(size(values, 1), 1) + group_counter];
    group_counter = group_counter + 1;
end
