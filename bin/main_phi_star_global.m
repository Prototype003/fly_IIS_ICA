            ...
            for tau_counter = 1 : length(taus)
                ...
                for channel_set = 1 : size(channel_sets, 1)
                    ...
                    for condition = 1 : size(fly_data, 5)
                        % Concatenate trials together to get original 18s
                        trials_concatenated = zeros(size(channel_sets, 2), size(fly_data, 1) * size(fly_data, 3));
                        sample_counter = 1;
                        % Note that phistar values will be identical across all trials (as they all use the same 18000 sample covariances)
                        for trial = 1 : size(fly_data, 3)
                            trials_concatenated(:, sample_counter:sample_counter+size(fly_data, 1)-1) = fly_data(:, channel_sets(channel_set, :), trial, fly, condition)';
                            sample_counter = sample_counter + 2250;
                        end
                        % Compute covariance
                        [cov_past_past, cov_past_present, cov_present_present] = Cov_comp(trials_concatenated, tau);
...
save([results_directory results_filename '_global.mat'], 'phis');
